...
async function getNotes(userId) {
    await fetch(`${baseUrl}/user/${userId}`, {
        method: 'GET',
        headers
    }).then(res => res.json()).then(data => createNoteCards(data)).catch(err => console.error(err))
}
...
async function getNoteById(noteId){
    await fetch (`${baseUrl}/${noteId}`, {
        method: 'GET',
        headers
    }).then(res => res.json()).then(data => populateModal(data)).catch(err => console.error(err))
}
    ...
    array.forEach(obj => {
        ...
        noteCard.innerHTML = `
            <div class="card d-flex" style="width: 18rem; height: 18rem;">
                <div class="card-body d-flex flex-column  justify-content-between" style="height: min-content">
